...
%}
    ...
    if tplus1 < win_size + 1
        x_tplus1 = 1 .* (data(tplus1, :)) - r(tplus1) .* w(tplus1, :);
    else
        closebefore = close_price((tplus1 - win_size + 1):(tplus1), :);
        closepredict = max(closebefore);
        %closepredict=(a.*closebefore(5,:)+a.*(1-a).*closebefore(4,:)+a.*(1-a)^2.*closebefore(3,:)+a.*(1-a)^3.*closebefore(2,:)+a.*(1-a)^4.*closebefore(1,:));
        x_tplus1 = 1 .* (closepredict ./ close_price(tplus1, :)) - r(tplus1) .* w(tplus1, :);
