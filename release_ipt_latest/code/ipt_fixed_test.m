function ipt_fixed_test(varargin)
    ...
    summary = struct('dataset', {}, 'T', {}, 'N', {}, ...
        ...
        'L_percentile', {}, 'q_value', {}, 'risk_factor', {});
    ...
    for i = 1:numel(files)
        ...
        if tie_factors
            [WI, RI, LI, QI, FI, UMI, TI, CI] = ndgrid( ...
                1:num_weights, 1:num_risks, 1:numel(L_percentiles), 1:numel(q_values), 1:numel(factor_values), 1:num_update_mix, 1:numel(max_turnover_list), 1:numel(q_clip_list));
            combos = [WI(:), RI(:), LI(:), QI(:), FI(:), UMI(:), TI(:), CI(:)];
        else
            [WI, RI, LI, QI, REVI, RFI, UMI, TI, CI] = ndgrid( ...
                1:num_weights, 1:num_risks, 1:numel(L_percentiles), 1:numel(q_values), 1:numel(factor_values), 1:numel(factor_values), 1:num_update_mix, 1:numel(max_turnover_list), 1:numel(q_clip_list));
            combos = [WI(:), RI(:), LI(:), QI(:), REVI(:), RFI(:), UMI(:), TI(:), CI(:)];
        end
        ...
        if fid ~= -1
            ...
            fprintf(fid, 'best params: L_percentile=%.1f, weight_inspect_wins=%d, risk_inspect_wins=%d, q_value=%.2f, risk_scale=%.4g\n', ...
                best.L_percentile, best.weight_inspect_wins, best.risk_inspect_wins, best.q_value, best.risk_factor);
            fclose(fid);
        end
...
function [wealth, max_drawdown, turnover_mean, sharpe] = eval_ipt_segment(data, p_close, w_YAR, Q_factor, win_size, tran_cost, epsilon, start_idx, end_idx, update_mix, max_turnover, sharpe_annualization, adaptive_inertia_q)
    ...
    % Initialize defensive_active and cap_active tracking
    defensive_active_count = 0;
    cap_active_count = 0;
    cap_bind_count = 0;
    Q_sum = 0;
    Q_count = 0;
    defensive_active_series = false(T, 1);
    cap_active_series = false(T, 1);
    cap_bind_series = false(T, 1);
    ...
    for t = 1:end_idx
        ...
        if t < end_idx
            ...
            if defensive_active
                defensive_active_count = defensive_active_count + 1;
            end
            ...
            cap_active = ~isinf(cap);
            cap_active_series(t) = cap_active;

            if cap_active
                cap_active_count = cap_active_count + 1;
            end
            ...
            if ~isinf(cap)
                ...
                if delta_turnover > 0
                    ...
                    if delta_turnover > cap
                        cap_bind_series(t) = true;
                        cap_bind_count = cap_bind_count + 1;
                    end
    ...
    % Calculate defensive_active and cap_active statistics
    defensive_active_pct = defensive_active_count / end_idx * 100;
    cap_active_pct = cap_active_count / end_idx * 100;

    if cap_active_count > 0
        cap_bind_pct = cap_bind_count / cap_active_count * 100;
    else
        cap_bind_pct = 0;
    end

    % Calculate Q factor mean
    if Q_count > 0
