...
for di = 1:numel(dataset_names)
            ...
            for risk = grid_risk
                ...
                for clip = grid_clip
                    ...
                    if isinf(clip)
                        ...
                        Q_factor = min(Q_factor_raw, clip);
                    end
                    
                    for mix = grid_mix
                        % Run Core
                        [~, daily_ret_val] = ipt_run_core(dev_data, w, p_struct.tran_cost, ...
                            w_YAR, Q_factor, p_struct.epsilon, mix, p_struct.max_turnover);
                        
                        % Evaluate
                        if val_end > length(daily_ret_val), val_end = length(daily_ret_val); end
    ...
    if isinf(best_p.clip)
        ...
        Q_factor = min(Q_factor_raw, best_p.clip);
    end
        
    [cum_wealth, daily_ret] = ipt_run_core(data, best_p.win, p_struct.tran_cost, ...
        w_YAR, Q_factor, p_struct.epsilon, best_p.mix, p_struct.max_turnover);
